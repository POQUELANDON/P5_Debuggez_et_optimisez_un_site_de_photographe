(function ($) {
  $.fn.mauGallery = function (userOptions) {
    var options = $.extend($.fn.mauGallery.defaults, userOptions); var tagsCollection = []; return this.each(function () {
    ...
    createRowWrapper(element) { if (!element.children().first().hasClass("row")) { element.append('<div class="gallery-items-row row"></div>') } }, wrapItemInColumn(element, columns) {
      ...
      let activeImage = $(".lightboxImage").attr("src"); $("img.gallery-item").each(function () { if ($(this).attr("src") === $(".lightboxImage").attr("src")) { activeImage = $(this) } }); let activeTag = $(".tags-bar span.active-tag").data("images-toggle"); let imagesCollection = []; if (activeTag === "all") { $(".item-column").each(function () { if ($(this).children("img").length) { imagesCollection.push($(this).children("img")) } }) } else { $(".item-column").each(function () { if ($(this).children("img").data("gallery-tag") === activeTag) { imagesCollection.push($(this).children("img")) } }) }
      let index = 0, prev = null; $(imagesCollection).each(function (i) { if ($(activeImage).attr("src") === $(this).attr("src")) { index = i } }); prev = imagesCollection[index - 1]; $(".lightboxImage").attr("src", $(prev).attr("src"))
    }, nextImage() {
